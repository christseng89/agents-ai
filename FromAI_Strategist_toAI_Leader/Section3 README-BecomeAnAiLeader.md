...
欢迎来到第三单元中最核心的部分，或许也是整个简报中最重要的部分——**路线图**的讨论。

一个典型的 AI 路线图通常包含 **五个阶段**。
这些阶段和一般项目的阶段大体类似，但在 AI 项目中，它们又有一些**特别的差异**和**独特**之处。

实际上，在这五个阶段之前，往往还有一个 “**第零阶段**”。

- 也就是先制定商业案例（Business Case），为这个项目获得高层的初步批准，并估算大致的成本与收益。
- 这其实就是我们在第一单元做的事情——建立决策框架、投资决策框架。
- 这些基本就是在为后续整个流程打好商业基础。
...
路线图交付的第一阶段是 **规划阶段**。
这一步要真正坐下来思考：整个时间线怎么安排？
我们要如何配置资源？（这就是我们上一节课提到的内容）
要确保我们清楚，在哪个时间点需要什么样的**技能**，并预留出足够的时间去**外部或內部招聘**。

如果涉及 **购买（Buy）** 的决策，还包括选择供应商与合作伙伴。

同样非常重要的是，在这个阶段就要设定好 KPI、指标，以及预期的业务成果。
这些将成为我们衡量项目进展与最终成效的 **指引灯**。
...
第二阶段就是 **研究阶段**。
这正是 AI 项目与传统项目最不一样的地方。

AI 项目会有一整个专门的 **研发（R\&D）阶段**。
其中一个非常重要的部分就是 **数据整理（Data Curation）**：
搜集、清洗并准备好支撑项目成功所需的数据。

理想情况下，这些数据来自企业内部，是独家的、专有的数据。
如果内部没有，就需要制定数据获取的计划：

* 从供应商购买数据
* 自行投入人力采集数据
* 或者与像 Mail.com 这样的公司合作，让他们为你生成数据集

以上都是数据整理阶段需要做的事情。

接着进入 **可行性分析（Feasibility Analysis）**，
最后通常会以某种原型（Prototype）或试点（Pilot）收尾，证明这个想法具有可行性。
...
这里我把测试（Testing）也包含进来。
有人会把测试单独作为一个阶段，但我认为：

* 测试
* 上线前的最终确认（Sign-off）
* 正式投产（Deployment to Production）

以及非常重要的：

* **变革管理（Change Management）**
* 培训与推广（Training & Adoption）

都属于同一个阶段。
特别是在那些风险较高的 AI 项目里，\*\*用户采用（Adoption）\*\*是常见的难点。
因此，从一开始就要把 Adoption 纳入规划、制定衡量方式，并提前管理，这是至关重要的。
...
AI 项目有很多不确定性，所以需要在这个阶段不断监测，并根据结果 **持续优化**。

---

在这五个阶段中，我特别用蓝色标出的 **第二阶段（Research）** 和 **第五阶段（Measure）**
是 AI 项目中最独特、也最与众不同的地方。
这两部分通常在传统 IT 项目里并不那么突出。

---

在技术领域，我们常听说两种项目管理方式的对比：

* **瀑布式（Waterfall）**
* **敏捷式（Agile）**

瀑布式就是传统的方法：
先需求分析，再设计，再开发，再测试，阶段分明。

而敏捷则是更迭代、更灵活的方式：
通常把任务切成 2-3 周的小周期，称为 **Sprint（冲刺）**。
每个 Sprint 都会有可交付成果，方便反馈、调整优先级。

你可能会想，我刚刚讲的五个阶段，看起来好像是典型的瀑布式？
其实，这整个流程是可以迭代的（Iterative）：

* 第零阶段（Business Case）可能只做一次
* 但接下来的五个阶段，可以完整跑一遍，产出一个 **MVP（最小可行产品）**
* 接着进入第二轮，继续迭代，产出 V2、V3…

在大型企业环境里，因为需要多团队协作，通常还是会按阶段进行，比如：
...
但在 **第三阶段（Build）** 内部，完全可以采用敏捷方式，
比如：

> Build 阶段是两个月，但内部是连续的 2 周 Sprint。
...
> 我们需要制定一个路线图（Roadmap），什么时候能交付哪些成果？

科学家们往往会翻个白眼，说：

> 我们都不确定这是不是做得到，更别提承诺日期了！
...
> “什么时候能做好？我们等到知道的那天再告诉你。”

所以，我们必须同时：

* 一只脚站在科学团队那边，理解不确定性
* 另一只脚站在管理层，给出相对清晰的时间线
...
> OpenAI 就发布了 **Agents SDK**

如果你正打算做大规模 Agent 项目，
这可能瞬间改变你的整个项目规划。

因此，在路线图中留出应对行业快速变化的时间是非常必要的。
...
> 有一些技巧，比如用 **LLM 充当测试评审员**
> 用 AI 来测试 AI

这是我们之前讲过的 **AI Patterns** 之一。
这类方法就是帮助你，在不确定性下，依然能做出稳健的测试。

不过，这需要团队里具备 AI 测试的专业人才。

---

### 用户采用（Adoption）：AI 项目的必修课

用户采用问题（Adoption）在所有项目中都存在，
但在 AI 项目里尤为突出。
...
✅ **跨部门沟通**
这不是单纯的技术问题，而是全公司共同的事。
大家都要被纳入信息流，尤其是要让人们知道有哪些风险。
